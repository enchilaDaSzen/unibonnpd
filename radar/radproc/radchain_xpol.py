...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug  7 11:29:16 2025
...
# =============================================================================
RSITES = ['Boxpol', 'Juxpol', 'Aaxpol']
RSITE = 'Boxpol'

RPARAMS = {rs['site_name']: rs for rs in RPARAMSALL
           if rs['site_name'] == RSITE}

print(f"elev_scans: {RPARAMS[RSITE]['scns']}")
RPARAMS[RSITE]['elev'] = 'n_ppi_110deg'

SCAN_ELEV = RPARAMS[RSITE]['elev'] if RSITE == 'Boxpol' else None

LPFILES = tpx.get_listfilesxpol(RSITE, START_TIME, STOP_TIME,
                                scan_elev=SCAN_ELEV,
                                parent_dir=PDIR)

# Overwrite the elev param for the volume scans (JXP and AXP)
SCAN_ELEV = None if RSITE == 'Boxpol' else RPARAMS[RSITE]['scns'].get('el_06')
...
rdata.ppi_xpol(scan_elev='sweep_0' if RSITE == 'Boxpol' else SCAN_ELEV)
...
# =============================================================================
if START_TIME.year > 2018 and rsnr.site_name == 'Juxpol':
...
if rnme.site_name == 'Boxpol':
    pathmfscl = MFS_DIR
    bcode = RPARAMS[rnme.site_name]['bclass']
    if rnme.scandatetime.year == 2017:
        clfmap = np.loadtxt(
            CLM_DIR + f'{rnme.site_name.lower()}{rnme.scandatetime.year}b'
            + '_cluttermap_el0.dat')
    elif rnme.scandatetime.year <= 2014:
        clfmap = None
        bcode = RPARAMS[rnme.site_name]['bclass'] - 64
        pathmfscl = None
    else:
        clfmap = np.loadtxt(
            CLM_DIR + f'{rnme.site_name.lower()}{rnme.scandatetime.year}'
            + '_cluttermap_el0.dat')
    if RPARAMS[rnme.site_name]['elev'] != 'n_ppi_010deg':
        bcode = RPARAMS[rnme.site_name]['bclass'] - 64
elif rnme.site_name == 'Juxpol':
    pathmfscl = None
    bcode = 23
    clfmap = None
else:
...
rmlyr.ml_top = 3.33652644296308
rmlyr.ml_bottom = 2.643073957621891
...
zdr0 = RPARAMS[rozdr.site_name]['zdr_offset']

rozdr.offset_correction(rnme.vars['ZDR [dB]'], zdr_offset=zdr0,
...
rattc.attc_phidp_prepro(rdata.georef, rdata.params, rozdr.vars, rhohv_min=0.8,
...
att_alphax = [0.15, 0.45, 0.30]  # Moderate to heavy rain PARK
