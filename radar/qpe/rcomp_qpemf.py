...
START_TIMES = [
    dt.datetime(2017, 7, 24, 0, 0),  # 24hr [NO JXP]
    dt.datetime(2017, 7, 25, 0, 0),  # 24hr [NO JXP]
    dt.datetime(2018, 5, 16, 0, 0),  # 24hr []
    # dt.datetime(2018, 9, 23, 0, 0),  # 24 hr [NO JXP]
    # dt.datetime(2019, 5, 8, 0, 0),  # 24 hr [NO JXP]
    # dt.datetime(2019, 5, 11, 0, 0),  # 24hr [NO JXP]
    # dt.datetime(2019, 7, 20, 8, 0),  # 16 hr [NO BXP]
    # dt.datetime(2020, 6, 17, 0, 0),  # 24 hr [NO BXP]
    # dt.datetime(2021, 7, 13, 0, 0),  # 24 hr []
    # dt.datetime(2021, 7, 14, 0, 0),  # 24 hr [NO BXP]
    ]

# %%
for START_TIME in START_TIMES:
    if START_TIME == dt.datetime(2019, 7, 20, 8, 0):
        STOP_TIME = START_TIME+dt.timedelta(hours=16)
    else:
    ...
    EWDIR2 = '/media/enchiladaszen/enchiladasz/safe/bonn_postdoc/'
    ...
    COMPNAME = 'DWD4'
    RPARAMS = [rs for rs in RPARAMS if rs['site_name'] in RSITES]
    RES_DIR = (EWDIR2 + f"pd_rdres/{START_TIME.strftime('%Y%m%d')}"
               + "/qpe_rcomp_dwd/5min/")
    ...
    for cntf, ratcf in enumerate(
        ...
        if 'r_kdpopt' in rprods:
            rkdp_fit = {k1: tpx.rkdp_opt(
                ...
                resattc[k1].vars[zh_kdpo[robj.site_name]],
                mlyr=rmlyr[k1], rband=rbands[robj.site_name],
        ...
        if 'r_zopt' in rprods:
            rzh_fit = {k1: tpx.rzh_opt(
                ...
                mlyr=rmlyr[k1],
                maxpia=(20 if rbands[robj.site_name] == 'X' else 10),
