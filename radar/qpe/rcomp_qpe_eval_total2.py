...
for nplot in nplots:
    ...
    for (axg, rprodk) in zip(grid, [k for k in sorted(eval_rqp.keys())]):
        ...
        f1 = axg.scatter(eval_rng[rprodk], eval_rqp[rprodk], marker='o',
                         c=[fres['altitude [m]']], edgecolors='k',
...
if stat2plot == 'R_Pearson [-]' or stat2plot == 'KGE':
...
bnd['KGE'] = np.linspace(0.8, 1, 11)
dnorm = {}
dnorm['MAE'] = mplclr.BoundaryNorm(bnd['MAE'], colors.N, extend='max')
dnorm['RMSE'] = mplclr.BoundaryNorm(bnd['RMSE'], colors.N, extend='max')
dnorm['NRMSE [%]'] = mplclr.BoundaryNorm(bnd['NRMSE [%]'], colors.N,
                                         extend='both')
dnorm['NMB [%]'] = mplclr.BoundaryNorm(bnd['NMB [%]'], colors.N, extend='both')
dnorm['R_Pearson [-]'] = mplclr.BoundaryNorm(bnd['R_Pearson [-]'],
                                             colors.N, extend='min')
dnorm['KGE'] = mplclr.BoundaryNorm(bnd['KGE'], colors.N, extend='min')
...
if stat2plot == 'R_Pearson [-]':
    ...
    coll2 = colored_bar(theta, [st[stat2plot][0][1]
                                for k, st in qpe_stats2.items()],
                        width=np.radians((360/len(qpe_stats2))-1), bottom=0,
                        facecolors='None', alpha=1,  edgecolors=('k',),
                        label=f'{rcomp2}')
elif stat2plot == 'KGE':
    coll = colored_bar(theta,
                       ...
                       width=np.radians((360/len(qpe_stats))-1), bottom=0,
                       edgecolors='grey', cmap=colors, norm=dnorm[stat2plot])
    cb = fig.colorbar(coll, orientation='horizontal', location='top',
                      fraction=0.10, shrink=0.8,
                      extend=dnorm[stat2plot].extend, pad=.1)
    coll2 = colored_bar(theta, [st[stat2plot]['kge']
                                for k, st in qpe_stats2.items()],
                        width=np.radians((360/len(qpe_stats2))-1), bottom=0,
                        facecolors='None', alpha=1,  edgecolors=('k',),
                        label=f'{rcomp2}')
    ...
    coll = colored_bar(theta, [st[stat2plot] for k, st in qpe_stats.items()],
                       ...
                       width=np.radians((360/len(qpe_stats))-1), bottom=0,
                       edgecolors='grey', cmap=colors,
                       norm=dnorm[stat2plot])
    cb = fig.colorbar(coll, orientation='horizontal', location='top',
                      fraction=0.10, shrink=0.8,
                      extend=dnorm[stat2plot].extend, pad=.1)
    coll2 = colored_bar(theta, [st[stat2plot] for k, st in qpe_stats2.items()],
                        width=np.radians((360/len(qpe_stats2))-1), bottom=0,
                        facecolors='None', alpha=1,  edgecolors=('k',),
                        label=f'{rcomp2}')
    # plt.polar(theta4, [st[stat2plot] for k, st in qpe_stats2.items()],
    #           label=f'[{rcomp2.replace("rcomp_qpe", "QPE").upper()}]',
    #           linestyle='-', marker='.', c='k', alpha=0.3,)
    # plt.fill(theta4, [st[stat2plot] for k, st in qpe_stats2.items()],
    #          alpha=0.3, c='grey')
else:
    coll = colored_bar(theta, [st[stat2plot] for k, st in qpe_stats.items()],
                       z=[st[stat2plot] for k, st in qpe_stats.items()],
...
for k1, iev in qpe_stats_ev.items():
    if k1.startswith('20210714'):
        if stat2plot == 'R_Pearson [-]':
            plt.scatter(
                theta4, [[rp[stat2plot][0][1] for k2, rp in iev.items()]],
                c=[[rp[stat2plot][0][1] for k2, rp in iev.items()]],
                marker=ms2[k1], edgecolors='k', s=64, cmap=colors,
                norm=dnorm[stat2plot],
                # label=(k1 + f' [{rcomp.replace("rcomp_qpe", "QPE").upper()}]'))
                label=(k1))
        elif stat2plot == 'KGE':
            plt.scatter(
                theta4, [[rp[stat2plot]['kge'] for k2, rp in iev.items()]],
            ...
            plt.scatter(
                theta4, [[rp[stat2plot] for k2, rp in iev.items()]],
    ...
    if k1.startswith('20210714'):
        if stat2plot == 'R_Pearson [-]':
            ...
            plt.scatter(
                ...
                marker=ms3[k1], edgecolors='k', c='w', s=64,
                label=(k1 + f'\n[{rcomp2.replace("rcomp_qpe", "QPE").upper()}]'))
cb.ax.set_title(stat2plot + f' [{rcomp.replace("rcomp_qpe", "QPE").upper()}]',
                size=18)
...
# ax.legend()
if stat2plot == 'R_Pearson [-]':
    plt.rgrids(np.arange(0.1, 1.01, .2), angle=0, size=lblsz, fmt='%.1f', c='k')
    y = 1.19
elif stat2plot == 'KGE':
    plt.rgrids(np.arange(0.1, 1.01, .2), angle=0, size=lblsz, fmt='%.1f', c='k')
    y = 1.19
elif stat2plot == 'NRMSE [%]':
    plt.rgrids(np.arange(0, 110, 20), angle=0, size=lblsz, fmt='%.0f', c='k')
    y = 119
elif stat2plot == 'NMB [%]':
    plt.rgrids(np.arange(-50, 60, 25), angle=0, size=lblsz, fmt='%.1f', c='k')
    y = 69
elif stat2plot == 'RMSE':
    plt.rgrids(np.arange(0, 21., 5), angle=0, size=lblsz, fmt='%.0f', c='k')
    y = 23.8
else:
    plt.rgrids(np.arange(0, 21., 5), angle=0, size=lblsz, fmt='%.0f', c='k')
    y = 23.8
